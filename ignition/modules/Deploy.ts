...
async function main() {
  ...
  console.log("Deployer ETH balance:", ethers.formatEther(await deployer.provider!.getBalance(deployer.address)));

  const UtilityToken = await ethers.getContractFactory("contracts/UtilityToken.sol:UtilityToken");
  ...
  const RealEstateManager = await ethers.getContractFactory("contracts/RealEstateManager.sol:RealEstateManager");
  ...
  const RentalContract = await ethers.getContractFactory("contracts/RentalContract.sol:RentalContract");
